...
suite "prettyprint":
  test "pretty print a string":
    ...
    check(
      prettyprint("Foo(Bar(Baz()), Baq())", 16) ==
        """
      Foo(
          Bar(Baz()),
          Baq()
    ...
    check(
      prettyprint("Foo(Bar(Baz()), Baq())", 12) ==
        """
        Foo(
            Bar(
                Baz(
...
suite "parse":
  test "parse a paren expression to a tree":
    ...
    check(
      parse("Foo, Bar") ==
        @[Tree.init("Foo", none(ParenType), @[], ","), Tree.init(" Bar")]
    )
    ...
    check(
      parse("""Foo{"\""}""") ==
        @[Tree.init("Foo", some(ParenType.curlyBracket), @[Tree.init(""""\""""")])]
    )
    check(
      parse("Foo{`\"`}") ==
        @[Tree.init("Foo", some(ParenType.curlyBracket), @[Tree.init("`\"`")])]
